...
class ExchangeTransactions extends StatefulWidget {
  final GlobalKey<ScaffoldState> _scaffoldKey;

  const ExchangeTransactions(this._scaffoldKey, {Key key}) : super(key: key);

  @override
  State<StatefulWidget> createState() {
    return _ExchangeTransactionsState();
  }
}

class _ExchangeTransactionsState extends State<ExchangeTransactions> {
  ...
  Widget build(BuildContext context) {
    ...
    List<TransactionRecord> _transactionRecordListNegative =
    ...
    return CustomScrollView(
      slivers: <Widget>[
        _isTransactionRecordLoading
            ...
            : SliverList(
                delegate: SliverChildListDelegate(
                  _transactionRecordListNegative.isEmpty
                      ? [
                          Container(
                            width: MediaQuery.of(context).size.width,
                            // decoration: BoxDecoration(color: Colors.grey[200]),
                      ...
                      : List.generate(
                          _transactionRecordListNegative.length,
                          (index) {
                            return _buildTransactionRecordCard(
                                _transactionRecordListNegative[index], index);
                          },
                        ),
                ),
              ),
